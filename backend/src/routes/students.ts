...
router.get('/', async (req, res) => {
  ...
  const result = keyword
    ? await pool.query<StudentRow>(`${SELECT_STUDENT} WHERE s.name ILIKE $1 ORDER BY s.created_at DESC`, [`%${keyword}%`])
...
router.post('/', async (req, res) => {
  ...
  const created = await pool.query<{ id: string }>(
    ...
    RETURNING id`,
    [name, grade, gender, school ?? null, classId ?? null, phone ?? null, parentPhone ?? null, status, enrolledAt, req.user!.id]
  );
...
router.put('/:id', async (req, res) => {
  ...
  const updated = await pool.query<{ id: string }>(
    ...
    WHERE id = $10
    RETURNING id`,
    [name, grade, gender, school ?? null, classId ?? null, phone ?? null, parentPhone ?? null, status, enrolledAt, req.params.id]
  );
